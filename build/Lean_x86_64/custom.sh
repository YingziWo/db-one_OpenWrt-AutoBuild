...
# 替换go版本到1.22.x --> default (目前default是23.x) 2024.09.14更改
...
#
sed -i 's#192.168.1.1#192.168.4.2#g' $NET                                                    # 定制默认IP
...
sed -i "s/OpenWrt /Self Build $(TZ=UTC-8 date "+%Y.%m.%d") @ Yingziwo in GitHub /g" $ZZZ
